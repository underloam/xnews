...
- Repository links now use `underloam/xnews`, with project-neutral package author metadata. The npm package name and scope are unchanged.
...
- Digest items carry a short `ref`, and `collectXnewsEvidence` / `resolveXnewsCitation` turn a citation back into the record the tools returned. A news item's own id is `provider|guid|title` and runs past 260 characters, so a model asked to cite it truncates at the `provider|guid` boundary: measured live across three providers, no cited id matched the id the model had been shown, and every one matched once the trailing title was stripped. Citations are now 24 characters and resolve exactly — 4/4 on every provider — and the index still accepts a full id or a truncated prefix so existing callers keep working. The handle hashes the raw item rather than the summarized copy, so a host can recompute it from the record it holds even when the digest truncated the id. A prefix claimed by two records — the same guid republished under a revised headline — resolves to neither rather than to whichever arrived first, and an unresolved citation is left exactly as the model wrote it.
- `ref` is a reserved field derived from a namespaced identity — tool, provider, and the record's stable identifier (`id`, or `sourceId` on a `WorkRecord`). An event id is unique only within its provider, so an `xnews_events` `across` result carrying alert `1` from two providers would otherwise give both the same handle and resolve to whichever arrived first. A provider record carrying its own `ref` cannot choose the handle a citation resolves through, one handle presented for two identities resolves to neither, and a bare id claimed by two records resolves to neither. A row with no stable identifier — a data release row, an extracted page — carries no `ref`, because a handle invented from its contents would change whenever the provider changed a column; the analyst is told to cite those by the fields they do carry. `citationRef` takes `{ tool, provider, id }`. The handle is 96 bits, because provider content is attacker-influenced and grinding a 48-bit collision to make one item resolve as another is roughly 2^24 work.
...
[Unreleased]: https://github.com/underloam/xnews/compare/xnews-langgraph-v0.2.3...HEAD
[0.2.3]: https://github.com/underloam/xnews/compare/xnews-langgraph-v0.2.2...xnews-langgraph-v0.2.3
[0.2.2]: https://github.com/underloam/xnews/compare/xnews-langgraph-v0.2.1...xnews-langgraph-v0.2.2
[0.2.1]: https://github.com/underloam/xnews/compare/xnews-langgraph-v0.1.0...xnews-langgraph-v0.2.1
[0.1.0]: https://github.com/underloam/xnews/releases/tag/xnews-langgraph-v0.1.0
